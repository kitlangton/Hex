...
public enum TransformationType: Codable, Equatable, Sendable {
	...
	case llm(LLMTransformationConfig)
}
...
public struct Transformation: Codable, Equatable, Identifiable, Sendable {
	...
	public var name: String {
		...
		case .llm(let config): return "LLM: \(config.shortDescription)"
		}
	}
...
extension TransformationType {
	private enum CodingKeys: String, CodingKey {
		...
		case replaceText, addPrefix, addSuffix, llm
	}
	...
	public func encode(to encoder: Encoder) throws {
		...
		case .llm(let config):
			try container.encode(config, forKey: .llm)
		}
	}
	...
	public init(from decoder: Decoder) throws {
		...
		} else if let config = try? container.decode(LLMTransformationConfig.self, forKey: .llm) {
			self = .llm(config)
		} else {
...
public struct LLMTransformationConfig: Codable, Equatable, Sendable {
	...
	public var shortDescription: String {
		...
		let display = trimmed.isEmpty ? "LLM" : trimmed
		return display.count > 18 ? String(display.prefix(18)) + "…" : display
	}
}
